...
\long\def\inso@secondofthree#1#2#3{#2}
...
\long\def\inso@fi@gobbletoqstop\fi#1\inso@qstop{\fi}
...
% called by \inso@secondofthree
\def\inso@ifdimen@false{\inso@z@.\inso@firstofone}
% non-expandable test whether TeX understands something as a dimension; #1 must
% be either a number or a dimension. The arguments to \inso@secondofthree will
% be:
...
%     #1=., #2=\inso@firstofone, #3=<true>
\protected\long\def\inso@ifdimen#1%>>=
  {%
    \afterassignment\inso@secondofthree
